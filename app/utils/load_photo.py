...
def crop_to_target_resolution(img, landscape_resolution=(2880, 2160), portrait_resolution=(2160, 2880)):
    ...
    cropped_img = img[y_margin:y_margin + target_height, x_margin:x_margin + target_width]
...
def loadPhoto(path):
    ...
    if puzzle_image.size//3 > 1280*720:
        if puzzle_image.shape[0] > puzzle_image.shape[1]:
            puzzle_image = cv2.resize(puzzle_image, (desired_resolution[1], desired_resolution[0]))
        else:
            puzzle_image = cv2.resize(puzzle_image, (desired_resolution[0], desired_resolution[1]))
    return puzzle_image